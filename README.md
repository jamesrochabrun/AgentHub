...
**Sparse checkouts for subdirectory launches.** For launched agent worktrees that start from a tracked subdirectory, AgentHub creates the worktree with `git worktree add --no-checkout`, infers a sparse owner path from project markers near the launch path, configures sparse checkout before materializing files, then starts the agent in the requested subdirectory. Inferred profiles include shared agent/tooling support paths when those paths exist, and Gradle support when a Gradle project marker is detected.

**No sparse surprises for root launches.** Repo-root launches keep the old full-checkout behavior unless the caller passes an explicit sparse profile. Subdirectory launches can still request `fullCheckout` when the whole repository must be materialized.
