...
Agent sessions drive the simulator through the bundled MCP tools: `agenthub_simulator_run` queues a Build & Run, waits for the outcome, and returns build errors (routing through an open panel via `SimulatorAgentRunRegistry` so it arms hot reload); `agenthub_simulator_screenshot` / `agenthub_simulator_describe_ui` are the verification loop; `agenthub_simulator_tap` / `_swipe` / `_type` / `_press_button` navigate the running app via bundled HID injection (`SimulatorUIDriver`, element-targeted taps through the AX tree — no idb/AXe install); and the panel auto Build & Runs on Swift changes when injection isn't armed. While the panel mirrors, the real Simulator.app window is ⌘H-hidden (`SimulatorAppHider`, default-on setting) and panel launches skip foregrounding it (`foregroundSimulatorApp: false`). Read the **Agent-driven runs & the verification loop**, **Auto Build & Run**, and **Hiding the real Simulator.app** sections of `SimulatorPreview.md` before editing `SimulatorRunRequest*`, `SimulatorRunResultStore`, `SimulatorAgentRunRegistry`, `SimulatorPanelRunFlow`, `SimulatorUIDriver`, or the simulator tool handlers in `AgentHubMCPServer`.
