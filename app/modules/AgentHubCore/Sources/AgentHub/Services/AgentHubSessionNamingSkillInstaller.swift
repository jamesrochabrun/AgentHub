...
enum AgentHubSessionNamingSkillInstaller {
  ...
  enum InstallError: LocalizedError {
    ...
    var errorDescription: String? {
      switch self {
      case .missingBundledSkill:
        return "Missing bundled AgentHub session naming skill."
      case .invalidBundledSkillEncoding:
        return "Bundled AgentHub session naming skill is not valid UTF-8."
      case .missingBundledOpenAIMetadata:
        return "Missing bundled AgentHub session naming OpenAI metadata."
      case .invalidBundledOpenAIMetadataEncoding:
        return "Bundled AgentHub session naming OpenAI metadata is not valid UTF-8."
      }
    }
  }
  ...
  static func installBundledSkillForAllProvidersBestEffort(
    ...
    do {
      ...
      AppLogger.session.error("Failed to install AgentHub session naming skill: \(error.localizedDescription)")
    }
  }
  ...
  static func installBundledSkillForAllProviders(
    ...
    guard let skillURL = bundle.url(
      forResource: "SKILL",
      withExtension: "md",
      subdirectory: "AgentHubSessionNamingSkill"
    ) else {
    ...
    guard let openAIYAMLURL = bundle.url(
      forResource: "openai",
      withExtension: "yaml",
      subdirectory: "AgentHubSessionNamingSkill/agents"
    ) else {
